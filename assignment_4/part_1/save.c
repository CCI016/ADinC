...
int treeForm(List *lp, FormTree *t) {
  if ( !treeLiteral(lp,t) ) {
    return 0;
  }
...
int treeFormula(List *lp, FormTree *t) {
  int binary = 0, biconditional = 0, and = 0;
	FormTree tL = NULL, tR = NULL;
	Token tok;
  if (!treeLiteral(lp,t)) {
    return 0;
  }
  while (*lp != NULL) {
    while ((and = acceptCharacter(lp,'&')) || acceptCharacter(lp,'|')) {
			tL = *t;
      tR = NULL;
      if ( !treeLiteral(lp,&tR) ) {
        freeTree(tR);
